...
const Login = () => {
  ...
  const nicknameExistModal = () => {
    toggleModal({
      title: "Регистрация",
      body: "Данный никнейм уже используется. Выберите другой",
    });
  };
  ...
  const fillReqfieldsModal = () => {
    ...
    return toggleModal({
      title: "Регистрация",
      body: `Заполните обязательные поля формы: ${requiredFiels.join(", ")}`,
    });
  };
  ...
  const friendClass = (id) =>
    cn(styles.friendList__slideProfile, {
      [styles.friendList__slideProfileActive]: registrationData?.friends?.find(
        (friend) => friend === id
      ),
    });
  ...
  useEffect(() => {
    if (!isEmpty(registrationData.name) && !isEmpty(registrationData.surname)) {
      ...
      setRegistrationData((state) => ({
        ...state,
        nickname: `${tr(name)}-${tr(surname)}`.toLowerCase(),
      }));
    }
